...
@section('title', 'Secțiune Nouă - Admin Panel')
@section('page_title', 'Secțiune Nouă')
@section('breadcrumbs')
...
<div class="p-6">
    ...
    <h1 class="text-2xl font-bold mb-6">Secțiune Nouă</h1>
    ...
    <form method="POST" action="{{ route('admin.homepage-sections.store') }}" class="bg-white rounded-lg shadow p-6">
        @csrf
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <select name="type" id="section-type" required class="w-full px-4 py-2 border border-gray-300 rounded-lg" onchange="showConfigFields()">
                        <option value="slider">Slider Bannere</option>
                        <option value="banners_row">Rând Bannere</option>
                        <option value="products_grid">Grid Produse</option>
                        <option value="categories_grid">Grid Categorii</option>
                        <option value="custom_html">HTML Custom</option>
                        <option value="video">Video</option>
                        <option value="text_block">Bloc Text</option>
                    </select>
                    @error('type')
            ...
            <div class="space-y-4">
                ...
                <div id="config-slider" class="config-field">
                    ...
                    <select name="config[position]" class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                        ...
                        <option value="middle">Middle (Mijloc)</option>
                        <option value="bottom">Bottom (Jos)</option>
                        <option value="footer">Footer (Subsol)</option>
                    </select>
                </div>
                ...
                <div id="config-products_grid" class="config-field hidden">
                    ...
                    <input type="number" name="config[limit]" value="{{ old('config.limit', 8) }}" min="1" max="20"
                ...
                <div id="config-categories_grid" class="config-field hidden">
                    ...
                    <input type="number" name="config[limit]" value="{{ old('config.limit', 6) }}" min="1" max="12"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                ...
                <div id="config-custom_html" class="config-field hidden">
                    ...
                    <textarea name="config[html_content]" rows="10"
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg font-mono text-sm">{{ old('config.html_content') }}</textarea>
                </div>
                
                <div id="config-video" class="config-field hidden">
                    <label class="block text-sm font-medium text-gray-700 mb-1">URL Video</label>
                    <input type="url" name="config[video_url]" value="{{ old('config.video_url') }}"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                    
                    <label class="block text-sm font-medium text-gray-700 mb-1 mt-4">Titlu Video</label>
                    <input type="text" name="config[video_title]" value="{{ old('config.video_title') }}"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                    ...
                    <textarea name="config[video_description]" rows="3"
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg">{{ old('config.video_description') }}</textarea>
                </div>
                ...
                <div id="config-text_block" class="config-field hidden">
                    ...
                    <textarea name="config[text_content]" rows="5"
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg">{{ old('config.text_content') }}</textarea>
                    
                    <label class="block text-sm font-medium text-gray-700 mb-1 mt-4">Culoare Fundal</label>
                    <input type="color" name="config[background_color]" value="{{ old('config.background_color', '#ffffff') }}"
                    ...
                    <input type="color" name="config[text_color]" value="{{ old('config.text_color', '#000000') }}"
                           class="w-full h-10 px-4 py-2 border border-gray-300 rounded-lg">
                </div>
            </div>
        </div>
