...
@section('title', 'Editează Utilizator - Admin Panel')
...
<div class="p-6">
    ...
    <h1 class="text-2xl font-bold mb-6">Editează Utilizator</h1>

    <form method="POST" action="{{ route('admin.users.update', $user->id) }}" class="max-w-2xl bg-white rounded-lg shadow p-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 gap-4">
            ...
            <div>
                ...
                <select name="role" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                    ...
                    <option value="{{ $role->name }}" {{ old('role', $user->role?->name) === $role->name ? 'selected' : '' }}>{{ ucfirst($role->name) }}</option>
                    @endforeach
                </select>
            ...
            <div class="flex items-center gap-4">
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="is_active" {{ old('is_active', $user->is_active) ? 'checked' : '' }} class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
        ...
        <div class="mt-6 flex gap-3">
            <button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Salvează modificările</button>
